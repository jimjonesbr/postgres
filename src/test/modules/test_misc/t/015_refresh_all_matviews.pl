...
($ret, $out, $err) = $node->psql(
    $db, 'REFRESH ALL MATERIALIZED VIEWS VERBOSE',
    extra_params => [ '--username' => 'u1' ]);
is($ret, 0, 'unprivileged REFRESH ALL succeeds as a no-op');
like($err, qr/skipping "public\.mv3": permission denied/,
     'VERBOSE reports skipped matview for unprivileged role');
...
like($err, qr/skipping "public\.mv1": materialized view cannot be refreshed concurrently/,
...
like($err, qr/CONCURRENTLY and WITH NO DATA options cannot be used together/,
