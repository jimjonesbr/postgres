...
# replication protocol: connetion must fail, since SQL commands
# cannot be executed.
$standby_live->psql(
	'postgres',
	"SELECT current_setting('port')",
...
like($stderr, qr/cannot execute SQL commands in WAL sender for physical replication/,
	"replication protocol set to 1");
