	...
	PG_TRY();
	{
		...
		forthree(arg, named_arg_strings, narg, xexpr->arg_names, nsarg, xexpr->xmlnamespaces)
		{
			...
			if (str && strlen(prefix) != 0)
			{
				 ...
				 *   prefix       - "xmlns" for both cases; libxml2 automatically
				 *                  renders this as the xmlns: attribute syntax.
				 *   name         - the namespace prefix for prefixed declarations,
				 *                  or "xmlns" itself for the default declaration.
